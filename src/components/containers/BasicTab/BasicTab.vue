...
export default {
    ...
    methods: {
        ...
        validate() {
            ...
            if (window.localStorage['fullname'] && window.localStorage['nickname'] && window.localStorage['email'] && window.localStorage['phone'] && window.localStorage['age'] && JSON.parse(window.localStorage['terms'])) {
                console.log("valid")
                this.next();
            }
        },
